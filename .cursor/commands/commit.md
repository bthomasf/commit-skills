...
2. 按顺序完成 7 步：分支检查（默认禁止 main/master，可配置）→ 变更检测 → 生成描述 → 用户确认/修改（含是否 Push）→ 写入 COMMIT-LOG.md → 统一暂存并执行 Commit → 登记活跃仓库与完成（按选择执行 Push）
3. 遇到终止条件时立即停止，不要跳过
...
- 当前分支为 main/master 时默认终止提交；可在项目根目录 `.commit-skill.json` 中配置 `protectedBranches` 覆盖
