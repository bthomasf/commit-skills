...
## 步骤 0：分支检查（必须最先执行，不可跳过）

**无论何种入口触发**（/commit 指令、用户说「帮我提交」等），**必须**在流程开始时最先执行本步骤，**禁止**跳过或延后。

1. 运行 `git branch --show-current` 获取当前分支名。
2. 读取项目根目录的 `.commit-skill.json`（若存在），取其中的 `protectedBranches`；否则使用默认 `["main", "master"]`。
3. 若当前分支在 `protectedBranches` 列表中（分支名**完全匹配**，如 `master`、`main`），则输出以下提示并**立即终止流程**，不执行后续任何步骤：
...
**防死循环**：若未暂存/已暂存变更**仅包含 COMMIT-LOG.md**（无其他代码文件），说明上次流程可能异常中断。此时应提示「检测到仅有 COMMIT-LOG.md 变更，请先手动 `git add . && git commit` 或 `git checkout -- COMMIT-LOG.md` 恢复后再提交」，**终止流程**，避免重复写入 COMMIT-LOG 导致死循环。
...
**防死循环**：本步骤**仅追加一条**记录（对应本次 commit message），**禁止**循环或重复追加。完成后**立即**进入步骤 5，**禁止**再次执行步骤 1～3 或重新检测变更。

---

## 步骤 5：统一暂存并执行 Commit

**防死循环（强制）**：步骤 4 完成后，**禁止**再次执行步骤 1～3、**禁止**重新运行 `git diff`/`git status` 做变更检测、**禁止**重新生成 message。**直接**依次执行 5.1 和 5.2，中间不做任何检测或分支。
